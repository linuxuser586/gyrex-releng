...
pull() {
        ...
        echo git pull
        git pull
        if ([[ $1 == *git.eclipse.org* ]] && ! [[ $1 == *forks* ]]); then
            gerritOriginRepoUrl=$(echo $1 | sed 's|ssh:.*@git.eclipse.org/gitroot|ssh://git.eclipse.org:29418|g' | sed 's|ssh://git.eclipse.org/gitroot|ssh://gerrit-local.eclipse.org:29418|g')
