...
tag_repo_commit () {
	...
	if ! ( git log -1  --format="%d" "$REPO_COMMIT" | grep "[ (]$NEW_TAG[,)]" >/dev/null); then
		echo "pushd \"$gitCache/$REPO_DIR\" ; git tag -a \"$NEW_TAG\" -m 'Build Submission' \"$REPO_COMMIT\" ; popd"
	fi
	popd >/dev/null
}
	...
	while read LINE; do
		...
		if ! ( git tag --contains $LAST_COMMIT | grep $CURRENT_TAG >/dev/null ); then
			...
			if ! ( git log -1  --format="%d" "$LAST_COMMIT" | grep "[ (]$NEW_TAG[,)]" >/dev/null); then
				echo "pushd \"$gitCache/$REPO_DIR\" ; git tag -a \"$NEW_TAG\" -m 'Build Submission' \"$LAST_COMMIT\" ; popd"
			fi
