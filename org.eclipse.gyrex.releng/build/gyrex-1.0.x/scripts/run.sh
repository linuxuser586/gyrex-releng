...
# This script is called from Hudson at build.eclipse.org and is archived (for example) in
# http://dev.eclipse.org/viewcvs/index.cgi/org.eclipse.gyrex/releng/org.eclipse.gyrex.releng/build/gyrex-1.0.x/scripts/?root=Technology_Project
# Build runs under ${WORKSPACE} == /opt/users/hudsonbuild/.hudson/jobs/cbi-*/workspace

# To use this script in Hudson you may visit https://build.eclipse.org/hudson/job/cbi-*/configure
...
./start.sh -projectid ${projectid} -version ${version} -buildType ${buildType} -buildTimestamp ${buildTimestamp} \
  -writableBuildRoot ${writableBuildRoot} -thirdPartyJarsDir ${thirdPartyJarsDir} -downloadsDir ${downloadsDir} -buildDir ${buildDir} -basebuilderBranch R36_M2 \
